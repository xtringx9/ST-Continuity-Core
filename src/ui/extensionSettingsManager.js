...
export function onNaiPresetSwitcherToggle(event) {
    const enabled = Boolean($(event.target).prop("checked"));
    const naiConfig = configManager.getNaiPresetSwitcherConfig();
    configManager.setNaiPresetSwitcherConfig({ ...naiConfig, enabled });
...
export function onChatu8LauncherToggle(event) {
    const enabled = Boolean($(event.target).prop("checked"));
    const naiConfig = configManager.getNaiPresetSwitcherConfig();
    configManager.setNaiPresetSwitcherConfig({
        ...naiConfig,
        chatu8Launcher: { ...(naiConfig.chatu8Launcher || {}), enabled },
    });

    if (enabled) {
        initChatu8Launcher();
        infoLog('[智绘姬文生图启动器] 功能已开启');
    } else {
        removeChatu8Launcher();
        infoLog('[智绘姬文生图启动器] 功能已关闭');
    }
}
...
function _saveAiGenerationConfig() {
    ...
    extensionConfig.module.asyncModule.askPromptBeforeGenerate = $('#continuity_ask_prompt_before_generate').prop('checked');
    extensionConfig.module.asyncModule.autoGenerateOnMessageEnd = $('#continuity_auto_generate_on_message_end').prop('checked');
